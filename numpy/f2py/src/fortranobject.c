    ...
    for (i=0;i<fp->len;i++) {
        if (fp->defs[i].rank == -1) {                      /* Is Fortran routine */
            ...
            if ((fp->defs[i].data)!=NULL) { /* Is Fortran variable or array (not allocatable) */
              PyArray_Descr * descr = get_descr_from_type_and_elsize(fp->defs[i].type, fp->defs[i].elsize);
              if (descr == NULL) {
                goto fail;
              }
              v = PyArray_NewFromDescr(&PyArray_Type, descr, fp->defs[i].rank, fp->defs[i].dims.d,
...
ndarray_from_pyobj(const int type_num,
    ...
    if (elsize < 0) {
      ...
      sprintf(mess + strlen(mess), " -- failed to determine element size from %s", Py_TYPE(obj)->tp_name);
    ...
    if ((intent & F2PY_INTENT_HIDE)
        ...
        if (!(intent & F2PY_INTENT_CACHE))
          PyArray_FILLWBYTE(arr, 0);  // TODO: should we fill fortran character arrays with blanks?
        return arr;
    }
...
static int
check_and_fix_dimensions(const PyArrayObject* arr, const int rank, npy_intp *dims, const char *errmess)
{
    /*
        ...
        for (i=0; i<rank; ++i) {
            ...
            if (dims[i]>=0) {
                if (d > 1 && d!=dims[i]) {
                    ...
                    sprintf(mess + strlen(mess), " -- %d-th dimension must be fixed to %"
                            NPY_INTP_FMT " but got %" NPY_INTP_FMT,
                            i, dims[i], d);
                    PyErr_SetString(PyExc_ValueError, mess);
                    return 1;
